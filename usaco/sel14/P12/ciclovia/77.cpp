...
int calculo(int x, int y, int d, int vieja){
    ...
    switch( d ){
        ...
        case 1:
            ...
            if( ol(vieja,2,0) ) costo+= CV[x].sentido == 0 ? 0 : 3;
        break;

        case 2:
            costo = CV[x].riesgo + ( CV[x].sentido == 0 ? 1 : 0 );
            if( ol(vieja,1,3) ) costo+= CV[x].sentido == 0 ? 0 : 3;
        break;

        case 3:
            costo = CH[y].riesgo + ( CH[y].sentido == 0 ? 1 : 0 );
            if( ol(vieja,0,2) ) costo+= CV[x].sentido == 0 ? 0 : 3;
        break;
    }
    return costo;
}
