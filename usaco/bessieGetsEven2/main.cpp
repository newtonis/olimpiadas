...
int main() {
    ...
    for (int b = 0;b < chances[0].size();b++){
        for (int i = 0;i < chances[3].size();i++){
            if ((chances[0][b]+chances[3][i] ) % 2 == 0){ continue; } //b+e+s+s+i+e seria par
            for (int g = 0;g < chances[4].size();g++){
                for (int o = 0;o < chances[5].size();o++){
                    for (int e = 0;e < chances[1].size();e++){
                        for (int s = 0;s < chances[2].size();s++){
                            if ((chances[4][g] + chances[5][o] + chances[1][e] + chances[2][s]) % 2 == 0){ continue; }//g+o+e+s seria par

                            for (int m = 0;m < chances[6].size();m++){
                                if (chances[6][m]%2 != 0){
                                    impar ++;
                                }
                            }
